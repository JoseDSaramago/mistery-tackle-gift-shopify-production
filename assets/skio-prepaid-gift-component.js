 ...
 *    <input type='hidden' name="properties[Gift Recipient's Email]" form='{{ block.settings.form_id }}'>
 *    <input type='hidden' name='properties[Gift Message]' form='{{ block.settings.form_id }}'>
 * </skio-prepaid-gift>
 *
...
export class SkioPrepaidGift extends LitElement {
  ...
  updateForm() {
    ...
    // These fields were added to show selected properties on the /cart and /checkout pages
    const $email_notification = this.querySelector('input[name="properties[Gift Recipient\'s Email]"]');
    const $message_notification = this.querySelector('input[name="properties[Gift Message]"]');

    if (!$email_notification || !$message) return;
    
    $email_notification.value = this.email;
    $message_notification.value = this.message;
    
    $email_notification.disabled = !this.isPrepaidGift || !this.isPrepaidSelected;
    $message_notification.disabled = !this.isPrepaidGift || !this.isPrepaidSelected;
  }
