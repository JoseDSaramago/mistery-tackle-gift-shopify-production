...
import{LitElement,html,css,unsafeHTML}from"https://cdn.jsdelivr.net/gh/lit/dist@3/all/lit-all.min.js";class SkioPrepaidGift extends LitElement{static properties={options:{type:Object},product:{type:Object},isPrepaidSelected:{type:Boolean},isPrepaidGift:{type:Boolean},debug:{type:Boolean},prepaidSelectedChange:{state:!0},form:{state:!0},email:{state:!0},message:{state:!0}};static styles=css`
  ...
  `;constructor(){super(),this.isPrepaidSelected=!1,this.isPrepaidGift=!0,this.form_id="",this.product={},this.form=null,this.email="",this.message="",this.sellingPlanInput=null,this.debug=window?.Shopify?.designMode}connectedCallback(){super.connectedCallback()}updated=e=>{e.has("isPrepaidSelected")&&this.isPrepaidSelected&&(this.prepaidSelectedChange=!0),e.has("options")&&this.options.form_id&&(this.form=document.querySelector("#"+this.options.form_id)||this.closest('form[action*="/cart/add"]'),this.form)&&(this.sellingPlanInput=document.querySelector(`[name="selling_plan"][form="${this.options.form_id}"]`),this.sellingPlanInput)&&(this.isPrepaidSelected=this.isPrepaidSellingPlan(this.sellingPlanInput.value),this.sellingPlanInput.addEventListener("change",e=>{this.isPrepaidSelected=this.isPrepaidSellingPlan(e.target.value)})),this.updateForm()};updateForm(){var e,i=this.querySelector('input[name="properties[_prepaidGiftEmail]"]'),t=this.querySelector('input[name="properties[_prepaidGiftMessage]"]');i&&t&&(i.value=this.email,t.value=this.message,i.disabled=!this.isPrepaidGift||!this.isPrepaidSelected,t.disabled=!this.isPrepaidGift||!this.isPrepaidSelected,i=this.querySelector('input[name="properties[Gift Recipient\'s Email]"]'),e=this.querySelector('input[name="properties[Gift Message]"]'),i)&&t&&(i.value=this.email,e.value=this.message,i.disabled=!this.isPrepaidGift||!this.isPrepaidSelected,e.disabled=!this.isPrepaidGift||!this.isPrepaidSelected)}isPrepaidSellingPlan(i){var e=this.product.selling_plan_groups.find(e=>e.selling_plans.find(e=>e.id==i));return!!e&&e.name.toLowerCase().includes("prepaid")}invalidIcon(){return html`
    ...
    `}render(){return html`
      ...
      ${this.isPrepaidSelected?html`
          ...
          `:""}
    `}}customElements.define("skio-prepaid-gift",SkioPrepaidGift);export{SkioPrepaidGift};